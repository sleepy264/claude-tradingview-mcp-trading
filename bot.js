...
async function getMaxLeverage(symbol) {
  ...
  return parseFloat(instrument.leverageFilter.maxLeverage);
}
...
async function placeBybitOrder(symbol, side, sizeUSD, price) {
  ...
  const orderBody =
    CONFIG.tradeMode === "spot"
      ...
      : {
          ...
          takeProfit: side === "buy" ? (price * 1.004).toFixed(2) : (price * 0.996).toFixed(2),
  ...
  const data = await res.json();
  if (data.retCode !== 0) {
    throw new Error(`Bybit order failed: ${data.retMsg}`);
  }
